...
namespace Log
{
    namespace
    {
        ...
        bool levelEnabledLocked(Level level)
        {
            if (level == Level::Error)
                return true;
            if (level == Level::Warning && alwaysShowWarnings)
                return true;
            ...
            case Level::Info:
                return level == Level::Error || level == Level::Warning || level == Level::Info;
            case Level::Debug:
        ...
        int nextLogIndex(const std::filesystem::path &directory)
        {
            ...
            for (std::filesystem::directory_iterator it(directory, error), end; !error && it != end;
                 it.increment(error))
            {
                if (!it->is_regular_file(error) || !isGeneratedLogFile(it->path()))
                    continue;
                const auto stem = it->path().stem().string();
                try
                {
                    result = std::max(result, std::stoi(stem.substr(0, stem.find('_'))) + 1);
                }
                catch (...)
                {
                }
        ...
        std::vector<std::string> enforceRetention(const std::filesystem::path &directory)
        {
            ...
            if (error)
                warnings.push_back("Could not scan log directory: " + error.message());
        ...
        void flushIfDueLocked()
        {
            if (!logFile.is_open() || flushIntervalSeconds <= 0.0)
                return;
            const auto now = std::chrono::steady_clock::now();
            if (std::chrono::duration<double>(now - lastFileFlush).count() < flushIntervalSeconds)
                return;
            logFile.flush();
            lastFileFlush = now;
        }
        ...
        void cleanupRateLimitsLocked(std::chrono::steady_clock::time_point now, bool forceForCapacity)
        {
            ...
            if (!cleanupDue && !forceForCapacity)
                return;
    ...
    void configureFromJson(const nlohmann::json &root)
    {
            ...
            try
            {
                ...
                if (logging.contains("always_show_warnings"))
                {
                    if (logging["always_show_warnings"].is_boolean())
                        ...
                        warnings.push_back("Invalid logging.always_show_warnings; using true");
                }
                    ...
                    if (logging["rate_limits"].is_object())
                    {
                        for (auto it = logging["rate_limits"].begin(); it != logging["rate_limits"].end(); ++it)
                        {
                            ...
                            if (!std::isfinite(value) || value < 0.0)
                                ...
                                configuredRateLimits[it.key()] = value;
                        }
                    }
                    else
                        warnings.push_back("Invalid logging.rate_limits; using an empty map");
                }
            }
        ...
        for (const auto &warningText : warnings)
            warning("LogConfig", warningText);
    }
    ...
    bool isRateLimited(const std::string &key, double seconds)
    {
        ...
        if (configured != configuredRateLimits.end())
            seconds = configured->second;
                               ...
                               const std::string &applicationName)
    {
        ...
        std::string failure;
        {
            ...
            try
            {
                ...
                if (!logFile)
                    throw std::runtime_error("could not open " + logPath.string());
                redirectStreams();
                std::call_once(shutdownRegistration, []
                               { std::atexit(shutdownFileLogging); });
        ...
        for (const auto &warningText : cleanupWarnings)
            warning("LogRetention", warningText);
    }
    ...
    ScopedTimer::~ScopedTimer() noexcept
    {
        ...
        try
        {
            const double elapsed = std::chrono::duration<double, std::milli>(
                                       ...
                                       .count();
            if (elapsed < minimumMilliseconds_)
                return;
